...
public class RabbitProducer {
    ...
    public static void main(String[] args) throws IOException, TimeoutException {
        ...
        channel.queueDeclare("Exchange_queue",true,false,false,null);
        channel.queueBind("Exchange_queue","Exchange_name","");
        Map<String, Object> exchangeMap = new HashMap<>();
        exchangeMap.put("alternate-exchange","Exchange_name");
...
//        channel.exchangeDeclare(RabbitUtils.EXCHANGE_NAME_DEMO1,"direct",true,false,map);
        channel.exchangeDeclare(RabbitUtils.EXCHANGE_NAME_DEMO1,"direct",true,false,exchangeMap);
...
//        queueMap.put("x-message-ttl",6000);
         ...
         */
        channel.queueBind(RabbitUtils.QUEUE_NAME_DEMO1,RabbitUtils.EXCHANGE_NAME_DEMO1,"");

        String message = "Hello World!";
        /**
         * basicPublish(String exchange, String routingKey, boolean mandatory, boolean immediate, BasicProperties props, byte[] body)
         * mandatory:true,交换器无法根据自身的类型和路由键找到一个符合条件的队列，RabbitMQ会调用basic.return将消息返回给生产者；false,直接丢弃。默认为false
         * immediate：true,如果交换器将消息路由到队列时发现队列上不存在任何消费者,那么这条消息将不会存入队列中。当与路由键匹配的所有队列都没有消费者时，RabbitMQ会调用basic.return将消息返回给生产者。默认为false
         */
...
//                .expiration("60000");   //设置单条消息的ttl，即使消息过期，也不会马上从队列中抹去，等到此消息即将被消费时再判定是否过期，如果过期再进行删除
        ...
        channel.basicPublish(RabbitUtils.EXCHANGE_NAME_DEMO1,RabbitUtils.ROUTING_KEY_DEMO1,true,false,builder.build(),message.getBytes("UTF-8"));
        channel.addReturnListener((replyCode, ReplyText, exchange, RoutingKey, basicProperties, bytes)->{
            ...
            logger.info("basic.return 返回的结果：{}",message);
        });
